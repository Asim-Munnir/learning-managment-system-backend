...
export const getCourseProgress = async (req, res) => {
    try {
        ...
        // step-01 fetch the user course progress
        let courseProgress = await CourseProgress.findOne({ courseId, userId }).populate({ path: "courseId" })
        ...
        // setp-02 if no progress found , return course details with empty progress
        if (!courseProgress) {
        ...
        // step-03 return the user's course progress along with course details
        return res.status(200).json({
            success: true,
            data: {
                courseDetails,
...
export const updateLectureProgress = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: "failed updated lecture progress"
        })
    }
}
